...
interface Props {
  ...
  markPaidItem: AuditItem | null;
  onMarkPaidClose: () => void;
  onMarkPaidConfirm: (item: AuditItem) => void;
...
export function PaymentAuditModals(props: Props) {
  ...
  return (
    <>
          ...
          {markUnpaidItem && (
            <div className="py-4 space-y-4">
              ...
              <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl text-xs text-amber-700 dark:text-amber-400 flex items-start gap-2">
                ...
                <span>
                  This reverses a manual (or synced) paid mark. It does not
                  refund CRM or Stripe. The installment becomes pending again so
                  you can re-invoice it.
                </span>
              </div>
            </div>
          )}
      ...
      {/* Modal: Mark Paid Confirmation (manual override) */}
      <Dialog
        open={!!markPaidItem}
        onOpenChange={(open) => !open && onMarkPaidClose()}
      >
        <DialogContent className="sm:max-w-[480px] rounded-3xl overflow-hidden shadow-xl border-border/40">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-xl font-bold text-emerald-600">
              <CheckCircle2 className="h-5 w-5" /> Mark this installment paid?
            </DialogTitle>
            <DialogDescription>
              Confirm you checked CRM (or the bank) and this payment actually
              posted.
            </DialogDescription>
          </DialogHeader>
          ...
          {markPaidItem && (
            <div className="py-4 space-y-4">
              <div className="bg-muted/40 p-4 rounded-2xl border border-border/50 space-y-2 text-sm">
                <InfoRow label="Client" value={markPaidItem.clientName} />
                <InfoRow
                  label="Installment"
                  value={markPaidItem.installmentLabel}
                />
                <InfoRow
                  label="Email"
                  value={markPaidItem.clientEmail || "Not provided"}
                />
                <div className="flex justify-between border-t border-border/40 pt-2 mt-2">
                  <span className="font-bold text-foreground">Amount:</span>
                  <span className="font-extrabold text-emerald-600 text-base">
                    {money(markPaidItem.installmentAmount)}
                  </span>
                </div>
              </div>
              <div className="p-3 bg-primary/5 border border-primary/10 rounded-xl text-xs text-muted-foreground space-y-1">
                <div className="font-semibold text-foreground flex items-center gap-1">
                  <ShieldCheck className="h-3.5 w-3.5 text-primary" /> Manual
                  override
                </div>
                <p>
                  This adds {money(markPaidItem.installmentAmount)} to the
                  wedding's paid total. It does not charge a card or create a
                  CRM invoice — confirm the money actually posted first.
                </p>
              </div>
            </div>
          )}
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button
              className="rounded-full bg-emerald-600 hover:bg-emerald-700 text-white"
              onClick={() => markPaidItem && onMarkPaidConfirm(markPaidItem)}
              disabled={markPaidPending}
            >
              {markPaidPending ? (
                <Loader2 className="h-4 w-4 animate-spin mr-2" />
